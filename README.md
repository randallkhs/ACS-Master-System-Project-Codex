...
- deterministic read-only Manual Review queue and single-item detail visibility with status/reason/severity/group counts, linked entity context, Water Emergency separation, age/attention visibility, reason/evidence context, chronological timeline evidence, audit/evidence references, and Randall-authorized Phase 0 review taxonomy metadata without review action execution
...
- dedicated read-only Manual Review Queue and detail sections that consume backend queue/detail contracts, separate Water Emergency-related reviews from standard dispatch/other reviews, and display reason/status/age/attention/entity/timeline evidence without approve/reject/defer/archive controls
