...
- deterministic read-only Manual Review queue details with status/reason/severity/group counts, entity context, Water Emergency separation, age/attention visibility, audit/evidence references, and Randall-authorized Phase 0 review taxonomy metadata without review action execution
...
- dedicated read-only Manual Review Queue section that consumes backend queue detail contracts, separates Water Emergency-related reviews from standard dispatch/other reviews, and displays reason/status/age/attention evidence without approve/reject/defer/archive controls
