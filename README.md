...
- deterministic read-only Manual Review queue and single-item detail visibility with status/reason/severity/group counts, filter/sort metadata, result-window metadata, frontend-only saved view preference support, linked entity context, Water Emergency separation, age/attention visibility, reason/evidence context, decision-readiness labels, action-preflight labels, future-action preview labels, future command-contract labels, audit-ledger dry-run labels, command-validation labels, safety-gate matrix visibility, expected non-binding outcome summaries, impacted entity references, future operator identity/role authorization/audit reason/idempotency/immutable event/post-action consistency requirement visibility, chronological timeline evidence, audit/evidence references, and Randall-authorized Phase 0 review taxonomy metadata without review action execution, validation execution, or audit writes
